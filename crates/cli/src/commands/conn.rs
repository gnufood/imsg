...
pub(crate) type Stream = Either<HubStream, bluer::rfcomm::Stream>;
...
/// public key.
pub(crate) fn resolve_hub_id(node_key: Option<&str>) -> Result<EndpointId> {
...
async fn connect_map_inner(
    ...
    match tgt {
        Target::Hub(id) => {
            ...
            session::lifecycle::establish_map_session(stream)
                .await
                .context("establishing MAP session over iroh")
        }
        Target::Rfcomm(addr, channel) => {
            let stream =
                transport::rfcomm::connect(addr, channel).await.context("RFCOMM connect (MAP)")?;
            session::lifecycle::establish_map_session(Either::Right(stream))
                .await
...
async fn connect_pbap_inner(
    ...
    match tgt {
        ...
        Target::Rfcomm(addr, channel) => {
            let stream =
                transport::rfcomm::connect(addr, channel).await.context("RFCOMM connect (PBAP)")?;
