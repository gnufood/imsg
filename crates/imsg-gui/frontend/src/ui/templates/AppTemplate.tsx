...
// Three-string union `App.tsx` can just redeclare structurally rather than needing a `.types.ts`.
...
interface AppTemplateProps {
  ...
  // Never knows these are real-IPC `MessagesConnected`/`ContactsConnected`/`SettingsConnected`
  // Wrappers.
  contactsSlot: React.JSX.Element
...
const AppTemplate = ({ contactsSlot, messagesSlot, onSelectScreen, screen, settingsSlot }: AppTemplateProps): React.JSX.Element => {
  ...
  return (
    <AppShellTemplate nav={nav}>
      {/* `mode="wait"` — the three screens' layouts differ enough that crossfading them
      ...
      <AnimatePresence initial={false} mode="wait">
        ...
        {screen === 'contacts' && (
          <motion.div key="contacts" animate={FADE_VISIBLE} className="h-full" exit={FADE_HIDDEN} initial={FADE_HIDDEN} transition={FADE_TRANSITION}>
            {contactsSlot}
          </motion.div>
        )}
        {screen === 'settings' && (
